...
Returned from Google during conversational turns, audio generation, and transcription. Google natively demarcates conversational turns using this payload.
...
```json
{
  "serverContent": {
    ...
    "outputTranscription": {
      "text": "That's great experience! Let's discuss your background with PostgreSQL."
    },
    "interrupted": false,
    "turnComplete": true
  }
...
#### Detailed Field Breakdown for Turn Mechanics:

| Field Path | Type | Role in Turn Definition |
| :--- | :--- | :--- |
| `serverContent.inputTranscription.text` | String | **User Turn Transcription**: Transcribed text of candidate speech captured by Google's real-time ASR when user speaks. |
| `serverContent.outputTranscription.text` | String | **Model Turn Transcription**: Transcribed text of AI speech generated by Gemini Live. |
| `serverContent.modelTurn.parts[].inlineData.data` | Base64 String | **Model Turn Audio**: Raw 24kHz single-channel PCM audio chunks streamed ~50 times/sec to client browser. |
| `serverContent.turnComplete` | Boolean | **Model Turn Complete Flag**: `true` when Gemini finishes synthesizing the current response turn and re-enters listening mode. |
| `serverContent.interrupted` | Boolean | **Barge-In Interruption Flag**: `true` when candidate speaks while AI is speaking, triggering audio flush on client. |

#### Conversational Turn Lifecycle Payload States:

##### State A: Candidate Speaking (User Turn Ingestion)
```json
{
  "serverContent": {
    "inputTranscription": {
...
##### State B: AI Streaming Response (Model Turn Generation)
```json
{
  "serverContent": {
    "modelTurn": {
      "parts": [
        {
          "inlineData": {
            "mimeType": "audio/pcm;rate=24000",
            "data": "UklGRiQAAABXQVZFZm10IBAAAAABAAEAwF0AAIC7AAACABAAZGF0YQAAAAA="
          }
        }
      ]
    },
    "outputTranscription": {
      "text": "That's great! Could you elaborate on your experience?"
    }
  }
}
```

##### State C: Turn Finalization (Model Turn Complete)
```json
{
  "serverContent": {
    "turnComplete": true
...
##### State D: Barge-In Interruption (Candidate Interrupts AI)
```json
{
  "serverContent": {
    "interrupted": true
