...
/**
 * DEEPGRAM NOVA-2 STT STRATEGY
 *
 * Provides WebSocket URL and header construction for Deepgram's Nova-2 general speech model.
 */
@Component
public class DeepgramNova2SttStrategy implements ISttProviderStrategy {

    public static final String STT_KEY = "DEEPGRAM_NOVA_2";

    @Override
    public boolean supports(String sttKey) {
        return STT_KEY.equalsIgnoreCase(sttKey);
    }
    ...
    public String buildWebSocketUrl(String apiKey, Map<String, Object> options) {
        ...
        return String.format(
            "wss://api.deepgram.com/v1/listen?encoding=%s&sample_rate=%d&channels=%d&model=nova-2-general&punctuate=true&interim_results=true&utterance_end_ms=%d&no_delay=true",
